...
export default function TarotView({ lang, state, setState, basiraContext }: any) {
    ...
    const submitFollowUp = async () => {
        ...
        try {
            const selected = cards.map((card, index) => ({ position: activeSpread.positions[index], name: isAr ? card.nameAr : card.name }));
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 pb-10">
            <section className="oracle-frame rounded-[28px] p-7 text-white">
                ...
                <p className="mt-3 max-w-sm text-sm leading-7 text-white/65">{isAr ? 'ثلاث بطاقات. ثلاث إشارات. بصيرة تربطها بسؤالك وتكشف لك الاتجاه الأقوى.' : 'Three cards. Three signals. Basira connects them to your question and surfaces the strongest direction.'}</p>
            </section>
            ...
            <section className="oracle-frame rounded-[24px] p-5">
                ...
                <div className="flex gap-2"><input value={question} onChange={event => setQuestion(event.target.value.slice(0, 300))} placeholder={isAr ? 'العلاقة، العمل، قرار يشغلك...' : 'A relationship, work, or a decision...'} className="min-w-0 flex-1 rounded-2xl border border-purple-100 bg-[#fbf9fc] px-4 py-3 text-sm outline-none focus:border-purple-300" /><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#291d3a] text-[#d9b96e]"><Send className="h-4 w-4" /></div></div>
                <div className="mt-4 grid grid-cols-3 gap-2" role="radiogroup" aria-label={isAr ? 'اختر نوع القراءة' : 'Choose a spread'}>{spreads.map(spread => <button key={spread.id} type="button" role="radio" aria-checked={spreadId === spread.id} onClick={() => { setSpreadId(spread.id); if (drawnCards.length || reading) reset(); }} className={`min-h-14 rounded-xl border px-2 py-2 text-xs leading-5 transition ${spreadId === spread.id ? 'border-[#d9b96e] bg-[#d9b96e]/15 text-[#f3d994]' : 'border-[#d9b96e]/20 bg-black/20 text-white/60 hover:border-[#d9b96e]/50'}`}>{spread.name}</button>)}</div>
            </section>

            <div className="grid grid-cols-3 gap-3 px-1">
                {cards.map((card, index) => {
                    const revealed = drawnCards.includes(index);
                    return <motion.button key={card.id} onClick={() => index === drawnCards.length && !isLoading && draw()} animate={{ y: revealed ? 0 : [0, -6, 0] }} transition={{ y: { repeat: Infinity, duration: 3, delay: index * .25 } }} className="relative aspect-[.62] [perspective:900px]">
                        <motion.div animate={{ rotateY: revealed ? 180 : 0 }} transition={{ duration: .8 }} className="relative h-full w-full [transform-style:preserve-3d]">
                            <div className="absolute inset-0 grid place-items-center overflow-hidden rounded-[18px] border border-[#d9b96e]/50 bg-[radial-gradient(circle,#654c7e,#171020_68%)] shadow-xl [backface-visibility:hidden]"><div className="absolute inset-2 rounded-[13px] border border-[#d9b96e]/25" /><span className="text-4xl text-[#d9b96e]">✦</span></div>
                            <div className="absolute inset-0 flex flex-col items-center justify-between rounded-[18px] border-2 border-[#d9b96e] bg-gradient-to-b from-[#fff8e8] to-[#ead9b8] p-3 text-center shadow-xl [backface-visibility:hidden] [transform:rotateY(180deg)]">
                                ...
                                <div className="absolute inset-x-1 bottom-1 rounded-b-[13px] bg-gradient-to-t from-black/90 to-transparent px-2 pb-2 pt-8 text-white"><strong className="block font-amiri text-xs">{isAr ? card.nameAr : card.name}</strong><span className="block text-[8px] text-white/75">{card.theme}</span></div>
                            </div>
                        </motion.div>
                    </motion.button>;
                })}
            ...
            {reading && <motion.section initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="oracle-frame rounded-[26px] p-5">
                ...
                <BasiraReadingText text={reading} dark />
                <div className="mt-5 rounded-2xl border border-[#d9b96e]/20 bg-black/20 p-4">
                    <textarea value={followUp} onChange={e => setFollowUp(e.target.value)} rows={3} className="w-full rounded-xl border border-[#d9b96e]/20 bg-black/20 p-3 text-sm text-white outline-none" placeholder={isAr ? 'أجب عن سؤال بصيرة أو اسأل عن نقطة محددة...' : 'Answer Basira’s question or ask about one specific point...'} />
                    <button onClick={submitFollowUp} disabled={!followUp.trim() || followUpLoading} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-[#d9b96e] py-3 text-sm font-bold text-[#171022] disabled:opacity-50"><Send className="h-4 w-4" />{followUpLoading ? '...' : (isAr ? 'قراءة أعمق' : 'Go deeper')}</button>
                    {followUpReply && <div className="mt-4"><BasiraReadingText text={followUpReply} dark /></div>}
                </div>
                <button onClick={reset} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl border border-[#d9b96e]/30 py-3 text-sm font-bold text-[#d9b96e]"><RotateCcw className="h-4 w-4" />{isAr ? 'قراءة جديدة' : 'New reading'}</button>
            </motion.section>}
...
function buildLocalReading(cards: TarotCard[], question: string, isAr: boolean) {
    if (!isAr) {
        return `[Strongest 3 signals]\n${cards.map((card, i) => `${i + 1}. ${card.name}: ${card.reflection}`).join('\n')}\n\n[Direction]\nThe three cards point to one symbolic direction${question ? ` around “${question}”` : ''}: watch what repeats, then choose your next move deliberately.`;
    }
    return `[أقوى 3 إشارات]\n${cards.map((card, i) => `${i + 1}. ${card.nameAr}: ${card.reflection}`).join('\n')}\n\n[الاتجاه]\nالبطاقات الثلاث ترسم اتجاهاً رمزياً واحداً${question ? ` حول «${question}»` : ''}: راقب ما يتكرر أمامك، ثم اختر خطوتك بوعي.`;
}
