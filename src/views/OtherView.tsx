...
import { ScanFace, Coffee, Crown, Eye, History, ShieldCheck, BookOpenCheck, MoonStar, BookHeart, Sparkles } from 'lucide-react';
        ...
        <motion.div initial={{ opacity: 0, x: 20, filter: 'blur(4px)' }} animate={{ opacity: 1, x: 0, filter: 'blur(0px)' }} exit={{ opacity: 0, x: -20, filter: 'blur(4px)' }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="flex flex-col gap-6 w-full pb-10 min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 px-1">
                ...
                <motion.div
                    ...
                    className="glass-card p-6 border-sky-200 shadow-sm rounded-2xl cursor-pointer bg-gradient-to-br from-sky-50/60 to-white flex items-center gap-4 group hover:shadow-md transition-shadow"
                >
                    <div className="w-14 h-14 rounded-full bg-sky-100/60 flex items-center justify-center border border-sky-200">
                        <BookOpenCheck className="text-sky-700 w-7 h-7" />
                    </div>
                    <div>
                        <h3 className="text-lg font-bold text-sky-900 font-amiri">{lang === 'ar' ? 'المنهج والمصادر' : 'Method & sources'}</h3>
                        <p className="text-xs text-gray-500 font-tajawal mt-1">{lang === 'ar' ? 'اعرف ما هو محسوب، بحثي أو تراثي' : 'See what is calculated, researched, or traditional'}</p>
                    </div>
                </motion.div>
