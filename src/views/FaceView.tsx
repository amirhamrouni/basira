        ...
        <motion.div initial={{ opacity: 0, x: 20, filter: 'blur(4px)' }} animate={{ opacity: 1, x: 0, filter: 'blur(0px)' }} exit={{ opacity: 0, x: -20, filter: 'blur(4px)' }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="flex flex-col gap-6 w-full pb-8">
            <div className="w-full h-48 -mt-4 mb-2 relative rounded-b-[40px] overflow-hidden shadow-md">
                ...
                <div className="absolute bottom-6 left-0 right-0 flex flex-col items-center z-10">
                    <h2 className="text-4xl font-bold text-stella-gold mb-2 text-center drop-shadow-sm font-amiri tracking-wide">{t.faceTitle}</h2>
                </div>
            </div>

            {error && <div role="alert" className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700 text-center">{error}</div>}
                ...
                {!imagePreview ? (
                    ...
                    <div className="w-full relative z-10 flex flex-col items-center py-4">
                        <div className="relative">
                            ...
                            {isScanning && <div className="absolute inset-0 bg-white/95 rounded-full flex flex-col items-center justify-center w-full h-full border-2 border-stella-gold/50 backdrop-blur-sm"><Loader className="w-10 h-10 text-stella-gold animate-spin mb-3" /><span className="text-sm text-stella-gold font-tajawal animate-pulse tracking-widest">{t.faceScan}</span></div>}
                        </div>
                        <button type="button" onClick={() => setState({ imagePreview: null, reading: null, isScanning: false, error: null })} className="mt-6 text-sm text-stella-gold/70 font-tajawal hover:text-stella-gold transition-colors font-bold tracking-wide">{t.retake}</button>
                    </div>
                )}
            </div>

            {reading && (
                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="glass-card p-5 bg-white border border-gray-100 shadow-sm rounded-[30px] relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 mb-5 relative z-10"><Sparkles className="w-6 h-6 text-stella-gold" /><h3 className="font-bold text-stella-gold font-amiri text-lg tracking-wide">{t.faceResultTitle}</h3></div>
                    <div className="relative z-10"><BasiraReadingText text={reading} /></div>
                </motion.div>
            )}
